  ...
  return (
    <div className="animate-slide-up">
      <div className="flex items-center justify-between mb-4 md:mb-6">
        ...
        <Link
          className="text-xs font-medium text-zinc-500 hover:text-primary transition-colors hover:scale-105 inline-block"
          href="/categories"
        >
          View all
        </Link>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4 stagger-children">
        {categoriesWithSlugs.map((category) => (
          <Link
            key={category.name}
            className="group p-3 md:p-4 bg-surface-dark rounded-xl border border-surface-border hover:border-primary/50 transition-all hover:bg-surface-hover hover:scale-105 flex flex-col items-center text-center md:items-start md:text-left"
